...
-- BOOK 테이블에서 책 제목이 '나'로 시작하는 정보 가져오기 
SELECT * FROM book
...
SELECT * FROM MEMBER
WHERE MEMBER_NAME LIKE '____';


# 문제1) member테이블에서 id가 qwer인 회원의 이름과 비밀번호 가져오기

SELECT MEMBER_NAME, MEMBER_PW
FROM MEMBER
WHERE MEMBER_ID = 'QWER';


# 문제2) book테이블에서 book_kind가 문학(100)인 책정보 가져오기
# 문학 필드(book_kind)
...
# 문제3) book테이블에서 가격이 15,000원 이하인 책정보 가져오기

SELECT * FROM BOOK
WHERE BOOK_PRICE <= 15000;

# 문제4) book테이블에서 문학(100)이 아닌 책정보만 가져오기
...
# 문제8) book테이블에서 문학(100)이거나 컴퓨터(300)인 책정보 가져오기

SELECT * FROM BOOK
WHERE BOOK_KIND = 100 OR 300;

# 문제9) book테이블에서 할인률이 없는 책정보 가져오기
#book_discount_rate 필드 이용

SELECT * FROM BOOK 
WHERE BOOK_DISCOUNT_RATE = 0;

# 문제10) book테이블에서 할인률이 있는 책정보 가져오기
